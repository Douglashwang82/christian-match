...
├── /messages                配對成功後的對話（後續迭代）
├── /profile/[id]            對象完整檔案（卡片點擊展開）
└── /me                      個人檔案與設定
```
...
主要可瀏覽路徑：`/`（首頁）→ `/register` → `/onboarding/church` → `/onboarding/survey` → `/match`（核心卡片頁）。

---

## 後續迭代建議 (Roadmap)

1. **配對後對話 `/messages`**：附「破冰經文 / 問題」引導深度交流。
